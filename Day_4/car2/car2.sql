...
SELECT 
    COUNT(maker) AS Total_no_of_maker, maker
FROM
    car2db.car2
...
SELECT 
    MAX(country) AS max_country_code
FROM
    car2db.cardb2;
...
SELECT 
    MIN(country) AS min_country_code
FROM
    car2db.cardb2;
...
SELECT 
    COUNT(maker) AS total_maker, country
FROM
    car2db.car2
GROUP BY country
ORDER BY COUNT(maker);
...
SELECT 
    COUNT(maker) AS total_maker, maker
FROM
    car2db.car2
WHERE
    country >= 3

...
SELECT 
    SUM(price) AS total_price_of_apple
FROM
    bakery2db.goods2
WHERE
    flavor = 'apple';
...
SELECT 
    AVG(price) AS avg_of_cake_flavor_food
FROM
    bakery2db.goods2
WHERE
    Food = 'cake';
...
SELECT 
    SUM(price) AS total_price, flavor
FROM
    bakery2db.goods2
GROUP BY flavor;
...
SELECT 
    AVG(price) AS average_price, Food
FROM
    bakery2db.goods2
WHERE
    Food = 'eclair' OR Food = 'cake'
GROUP BY Food;

-- 4. Display the maximum price of each Food from goods table ?

SELECT 
    MAX(price) AS max_price, Food
FROM
    bakery2db.goods2
GROUP BY Food;
